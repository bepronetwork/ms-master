...
async function addCurrencyWallet(req, res) {
    try{
        SecuritySingleton.verify({type : 'admin', req, permissions: ["super_admin", "financials"]});
        await MiddlewareSingleton.log({type: "admin", req});
        let params = req.body;
		let app = new App(params);
...
async function editTopBar (req, res) {
    try{
        SecuritySingleton.verify({type : 'admin', req, permissions: ["super_admin", "customization"]});
        await MiddlewareSingleton.log({type: "admin", req});
        let params = req.body;
		let app = new App(params);
