...
class Globals{
    ...
    async startDatabase(){
        ...
        await this.ecosystem_db.connect(MONGO_CONNECTION_STRING + "/ecosystem", { useNewUrlParser: true, useUnifiedTopology: true});
        this.ecosystem_db.Promise = bluebird;
        // Redis DB
        this.redis_db = new Mongoose();
        this.redis_db.set('useFindAndModify', false);
        await this.redis_db.connect(MONGO_CONNECTION_STRING + "/redis", { useNewUrlParser: true, useUnifiedTopology: true});
